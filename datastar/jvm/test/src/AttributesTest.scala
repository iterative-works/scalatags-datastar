...
object AttributesTest extends TestSuite:
    ...
    val tests = Tests:
        ...
        test("plain-form attributes render data-<plugin>"):
            ...
            assert(render(dataAttr := "{disabled: $busy}") == """<div data-attr="{disabled: $busy}"></div>""")
            assert(render(dataStyle := "{color: $c}") == """<div data-style="{color: $c}"></div>""")
            assert(render(dataBind := "query") == """<div data-bind="query"></div>""")
            assert(render(dataClass := "{active: $open}") == """<div data-class="{active: $open}"></div>""")
        ...
        test("keyed-form attributes render data-<plugin>:<key> with a literal colon"):
            assert(render(dataSignals("count") := "0") == """<div data-signals:count="0"></div>""")
            assert(render(dataComputed("total") := "$a + $b") == """<div data-computed:total="$a + $b"></div>""")
            assert(render(dataBind("query") := "") == """<div data-bind:query=""></div>""")
            assert(render(dataClass("active") := "$open") == """<div data-class:active="$open"></div>""")
            assert(render(dataAttr("disabled") := "$busy") == """<div data-attr:disabled="$busy"></div>""")
            assert(render(dataStyle("color") := "$c") == """<div data-style:color="$c"></div>""")
            assert(render(dataRef("el") := "") == """<div data-ref:el=""></div>""")
            assert(render(dataIndicator("loading") := "$busy") == """<div data-indicator:loading="$busy"></div>""")

        test("hyphenated event attributes are distinct plugins, not keyed forms"):
            assert(render(dataOnIntersect := "@get('/x')") == """<div data-on-intersect="@get('/x')"></div>""")
            assert(render(dataOnInterval := "@get('/x')") == """<div data-on-interval="@get('/x')"></div>""")
            assert(render(dataOnSignalPatch := "@get('/x')") == """<div data-on-signal-patch="@get('/x')"></div>""")
            assert(
            ...
            assert(render(dataPreserveAttr := "class") == """<div data-preserve-attr="class"></div>""")
            assert(render(dataJsonSignals := "$store") == """<div data-json-signals="$store"></div>""")
        ...
        test("timing modifiers render with duration and boolean flags"):
            ...
            assert(dataOnSignalPatch.debounce(100.millis).attrName == "data-on-signal-patch__debounce.100ms")
        ...
        test("attribute-specific modifiers"):
            ...
            assert(dataInit.delay(500.millis).viewTransition.attrName == "data-init__delay.500ms__viewtransition")
